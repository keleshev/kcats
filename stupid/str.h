...
// FIXME ass(str_endswith(s, "")); //hm...
bool str_endswith(char* s, char* end) {
    return str_eq(str_slice_auto(s, -1 * strlen(end), 0), end);
}

bool str_startswith(char* s, char* start) {
    return str_eq(str_sub_auto(s, 0, strlen(start)), start);
}
